...
function EmbedDemo() {
  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 72px)', background: '#f8f7f4', fontFamily: "'Outfit', 'DM Sans', sans-serif", color: '#1c1c1c' }}>
      ...
      <header style={{ background: '#1c2b3a', padding: '0 40px', height: '64px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <nav style={{ display: 'flex', gap: '24px' }}>
          {['Buy', 'Rent', 'Sell', 'About'].map((item) => (
            <span key={item} style={{ color: 'rgba(255,255,255,0.6)', fontSize: '13px', cursor: 'pointer' }}>{item}</span>
          ))}
        </nav>
      </header>
      ...
      {/* Photo gallery - NO position:relative, NO overflow:hidden on wrapper */}
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gridTemplateRows: '260px 260px', gap: '4px', maxHeight: '528px' }}>
        {ROOMS.map((room, idx) => (
      ...
      {/* Widget trigger button - BELOW the grid, not overlapping it */}
      <div style={{ background: '#f0ede6', padding: '16px 24px', display: 'flex', justifyContent: 'flex-end', borderBottom: '1px solid #e8dcc8' }}>
        <button
          onClick={openWidget}
          style={{
            background: '#1c2b3a',
        ...
        >
          ...
          <div style={{ textAlign: 'left' }}>
            ...
            <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.6)', letterSpacing: '0.08em' }}>AI Interior Styling</div>
          </div>
        </button>
      </div>
      ...
      {/* Embed code section */}
      <div id="embed-code" style={{ background: '#1a1612', padding: '72px 40px', marginTop: '40px' }}>
        <div style={{ maxWidth: '860px', margin: '0 auto' }}>
          <p style={{ fontSize: '11px', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#b8965a', fontWeight: 500, marginBottom: '12px' }}>Integration</p>
          <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '36px', fontWeight: 300, color: '#f5f0e8', lineHeight: 1.1, marginBottom: '32px' }}>
            One snippet. <em style={{ fontStyle: 'italic', color: '#d4b07a' }}>Every listing.</em>
          </h2>
          <div style={{ background: '#0f0e0c', border: '1px solid rgba(232,220,200,0.1)', borderRadius: '4px', padding: '28px', fontFamily: 'monospace', fontSize: '13px', lineHeight: 1.8, overflowX: 'auto' }}>
            <div style={{ color: '#8a9baa', marginBottom: '4px' }}>Place before body close tag in your listing template</div>
            ...
            <div style={{ color: '#a8d8a8', paddingLeft: '16px' }}>data-key="ag_live_your_key_here"</div>
      ...
      {widgetState !== 'closed' && (
        <div
          ...
          style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(26,22,18,0.6)', backdropFilter: 'blur(4px)', display: 'flex', justifyContent: 'flex-end' }}
        >
          <div style={{ width: '440px', maxWidth: '100vw', background: '#faf7f2', height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column', animation: 'slideIn 0.3s ease' }}>
            ...
            <div style={{ background: '#1a1612', padding: '24px 24px 20px', borderBottom: '1px solid rgba(232,220,200,0.1)', flexShrink: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                ...
                <button onClick={closeWidget} style={{ background: 'transparent', border: 'none', color: 'rgba(245,240,232,0.4)', cursor: 'pointer', fontSize: '20px', lineHeight: 1, padding: '4px' }}>X</button>
              </div>
            ...
            <div style={{ flex: 1, padding: '24px', overflowY: 'auto' }}>
              ...
              {widgetState === 'picker' && (
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '28px' }}>
                    {STYLES.map((style) => (
                      <div
                        ...
                        style={{ cursor: 'pointer', borderRadius: '2px', overflow: 'hidden', border: `2px solid ${selectedStyle === style.id ? '#b8965a' : 'transparent'}`, transition: 'all 0.2s', background: '#fff', boxShadow: '0 2px 12px rgba(26,22,18,0.07)' }}
                      >
                        <div style={{ height: '70px', background: style.gradient, position: 'relative' }}>
                          {selectedStyle === style.id && (
                            <div style={{ position: 'absolute', top: '6px', right: '6px', width: '20px', height: '20px', background: '#b8965a', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <button
                    ...
                    style={{ width: '100%', background: selectedStyle ? '#b8965a' : '#e8dcc8', color: '#fff', border: 'none', padding: '14px', borderRadius: '2px', fontSize: '13px', fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: selectedStyle ? 'pointer' : 'not-allowed', fontFamily: "'DM Sans', sans-serif", transition: 'background 0.2s' }}
                  >
                    Reimagine 5 rooms
                  </button>
                  <p style={{ fontSize: '11px', color: '#8a7f72', textAlign: 'center', marginTop: '12px' }}>AI Visualisation - not a guarantee of appearance</p>
                </>
              )}
              ...
              {widgetState === 'generating' && (
                <div style={{ textAlign: 'center', padding: '40px 0' }}>
                  ...
                  <p style={{ fontSize: '13px', color: '#8a7f72', marginBottom: '28px' }}>{STYLES.find((s) => s.id === selectedStyle)?.label} style</p>
                  {ROOMS.map((room, idx) => (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px', padding: '10px 12px', background: '#fff', borderRadius: '2px' }}>
                      <div style={{ width: '20px', height: '20px', borderRadius: '50%', background: generatingIdx > idx ? '#b8965a' : '#e8dcc8', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.4s' }}>
                        {generatingIdx > idx && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>}
                      </div>
                      <span style={{ fontSize: '13px', color: '#1a1612' }}>{room.label}</span>
                      {generatingIdx === idx && <span style={{ fontSize: '11px', color: '#b8965a', marginLeft: 'auto' }}>generating...</span>}
                      {generatingIdx > idx && <span style={{ fontSize: '11px', color: '#6a9a6a', marginLeft: 'auto' }}>done</span>}
                    </div>
                  ))}
                </div>
              )}
              ...
              {widgetState === 'results' && (
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                    ...
                    <button onClick={() => { setWidgetState('picker'); setSelectedStyle(null) }} style={{ background: 'transparent', border: 'none', fontSize: '11px', color: '#8a7f72', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
                      Try another style
                    </button>
                  </div>
                  <p style={{ fontSize: '12px', color: '#8a7f72', marginBottom: '16px' }}>
                    Styled in <strong style={{ color: '#1a1612' }}>{STYLES.find((s) => s.id === selectedStyle)?.label}</strong>. Compare before and after.
                  </p>
                  {ROOMS.map((room, idx) => (
